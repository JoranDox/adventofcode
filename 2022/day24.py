...
def gettimeslice(multitimelocset, t, printable=False):
    ...
    if printable:
        return {
            k: vsum(*[bliztomap(locset,i).get(k,(0,0,0,0)) for i,locset in enumerate(slices)])
            for k in keys
        }
    else:
        return keys
...
def solutiononeway(entryloc,exitloc,starttime=0):
    ...
    while priolisttodo:
        ...
        if steps % 10000 == 0:
            print("seen steps:", t, len(seenloctimes), steps, time.time()-tstart, steps / (time.time()-tstart))
        ...
        for direction in [(0,0)] + directions_tup:
            ...
            if (
                ...
                priolisttodo[manhattandistance((sx,sy),exitloc)+t].append(suggestion)
