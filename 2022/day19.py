...
for bname, blueprint in blueprints.items():
        ...
        for state in states:
            ...
            for rname, rcost in blueprint.items():
                ...
                if maxcosts[rname] > 0:  # avoid skipping geode bots
                    if (
                        r[rname] >= maxcosts[rname]
                    ):  # if we already have enough bots of this kind
                ...
                if nt is not None:
                    ...
                    if minute + nt + 1 <= maxmins:
                        ...
                        newstate = (
                            ...
                            vsum(
                                i, vmul(r, nt + 1), neg(rcost)
                            ),  # minus cost, plus working robots until then
                        )
    ...
    if bname == 1 and len(data.splitlines()) == 2:
        for minute, (r, i) in enumerate(example_states):
            if r != prevrobots:
                assert (r, i) in statesperminute[minute], (
                    r,
                    i,
                    statesperminute[minute],
                )
