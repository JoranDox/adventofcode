...
# with open(aoc_dir.joinpath("input/2022/day15inputtest.txt")) as f:
with open(aoc_dir.joinpath("input/2022/day15input.txt")) as f:
    data = f.read().strip()
...
minx = min(min([x for x, y in sensors]), min([x for x, y in beacons]))
maxx = max(max([x for x, y in sensors]), max([x for x, y in beacons]))
miny = min(min([y for x, y in sensors]), min([y for x, y in beacons]))
maxy = max(max([y for x, y in sensors]), max([y for x, y in beacons]))
...
def retoverlap(cmin, cmax, c1, c2):
    if (c2 < (cmin - 1)) or (c1 > (cmax + 1)):
        ...
        (c1 <= cmin - 1 <= c2) or (c1 <= cmax + 1 <= c2) or (c1 >= cmin and c2 <= cmax)
    ):
        # overlap
...
def intersect(r_edge, f_edge):
    ...
    rset = {(rx1 + i, ry1 - i) for i in range(rx2 - rx1 + 1)}
    fset = {(fx1 + i, fy1 + i) for i in range(fx2 - fx1 + 1)}
    # print(rset,fset)
...
def interestingpoints2():
    ...
    for i, ((sx1, sy1), d1) in enumerate(list(closest.items())):
        for j, ((sx2, sy2), d2) in enumerate(list(closest.items())[i + 1 :]):
            if manhattandistance((sx1, sy1), (sx2, sy2)) == d1 + d2 + 2 and (
                (i + j + 1) < len(closest)
            ):
                for k, ((sx3, sy3), d3) in enumerate(
                    list(closest.items())[i + j + 1 :]
                ):
                    if (
                        manhattandistance((sx1, sy1), (sx3, sy3)) == d1 + d3 + 2
                        and manhattandistance((sx2, sy2), (sx3, sy3)) == d2 + d3 + 2
                    ):
                        overlap = (
