            ...
            if locs:
                ...
                for rot in range(4):
                    ...
                    for loc, expected in zip(
                        [(locx+1, locy), (locx, locy+1), (locx-1, locy), (locx, locy-1)],
...
def enlarge(smallermap, cubemap):
    return {
        (x,y) : smallermap[(x // sidesize, y//sidesize)]
        for x,y in cubemap
    }
...
def nextposoncube(lazymap, location, direction):
    ...
    if suggested not in lazymap:
        ...
        dprint("topleft at", (topleftx, toplefty), "=", (topleftx * sidesize, toplefty * sidesize))
        relx,rely = vsum(location, neg((topleftx * sidesize, toplefty * sidesize)))

        dprint("pos, topleft, rel pos", location, (topleftx * sidesize, toplefty * sidesize), (relx, rely))
        ...
        dprint("nexttopleft at", (nexttopleftx, nexttoplefty), "=", (nexttopleftx * sidesize, nexttoplefty * sidesize))
        suggested = vsum(nextdenormalisedposition ,(nexttopleftx * sidesize, nexttoplefty * sidesize))
...
for instruction in parseinstructionstring(instructions):
    ...
    if instruction == "R":
        ...
        for i in range(instruction):
            ...
            currentposition, currentdirection = nextposoncube(cubemap, currentposition, currentdirection)
