...
# with open(parent_directory.joinpath("day12inputtest.txt")) as f:
with open(parent_directory.joinpath("day12input.txt")) as f:
    data = f.read().strip()
...
def tostrend(themap, path):
    return "\n".join([
        "".join([(chr(square) if ((sx,sy) in path) else ".") for sx,square in enumerate(line)])
        for sy,line in enumerate(themap)
    ])
def tostrseen(themap, part):
    return "\n".join([
        "".join([
            (str(seenlocs[part][(sx,sy)]).rjust(3) if ((sx,sy) in seenlocs[part]) else "...") for sx,square in enumerate(line)])
        for sy,line in enumerate(themap)
    ])
            ...
            for steps, sublist in enumerate(tochecklocs[part]):
                ...
                if sublist:
                    ...
                    for nx, ny in getneighbours(x, y):
                        ...
                        if nx < 0 or ny < 0 or nx >= len(themap[0]) or ny >= len(themap):
                            # print("\nnot on map")
                            continue
                        neigh = themap[ny][nx]
                        # print("neigh", neigh)
                        if neigh <= (currpos + 1):
                            ...
                            if (nx,ny) in seenlocs[part]:
                                if seenlocs[part][(nx,ny)] > steps + 1 and part == "p1":
                                    print("this shouldn't happen in p1", (nx,ny), "already seen with:", seenlocs[part][(nx,ny)], "but now I got here in", steps+1)
                                # print("already seen as", seenlocs[part][(nx,ny)])
                            if ((nx,ny) not in seenlocs[part]) or (seenlocs[part][(nx,ny)] > steps +1):
                                # print("yes")
                                newseen = True
                                seenlocs[part][(nx,ny)] = steps + 1
                                # check em (again)
                                tochecklocs[part][steps + 1].append(path_to_here + ((nx,ny),))
