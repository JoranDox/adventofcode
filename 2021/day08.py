...
subsets = [
    [ set(maps[n]).issubset(set(maps[n2])) for n2 in range(10)]
    for n in range(10)
]
print(f"{subsets}")

intersections = [
    [ set(maps[n]) & (set(maps[n2])) for n2 in range(10)]
    for n in range(10)
]
...
for d in data:
    ...
    mapping = {
        l:set("abcdefg") for l in "abcdefg"
    }
    # print(mapping)
    iters = 0
    progress = 0
    ...
    while not all([outdsplit in known_maps]):
        ...
        for n,l in enumerate(lens):
            ...
            candidates = {
                ...
                if all(
                    [ _c in potential_maps[n] for _c in c]
                )
            }
            # print(f"{candidates=}")
...
for d in data:
    ...
    mapping_rev = {a:b for a,b in zip(
        [i_[0] for i_ in c.most_common()],
        [i_[0] for i_ in c2.most_common()])
    }
