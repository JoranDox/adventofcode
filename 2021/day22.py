...
def cubeoverlap(c1,c2):
    return tuple(
        rangeoverlap(dim1,dim2)
        for dim1,dim2 in zip(c1,c2)
    )
...
for line in tqdm.tqdm(data.splitlines()):
    ...
    xrange,yrange,zrange = [
        tuple(map(int,coord.split("=")[1].split("..")))
...
for onoff, (xrange,yrange,zrange) in tqdm.tqdm(cubes):
    ...
    assert cubesize((xrange,yrange,zrange)) == len(subset),cubesize((xrange,yrange,zrange)) == len(subset)
