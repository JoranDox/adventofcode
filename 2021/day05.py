...
datahor = [((i, j), (k, l)) for ((i, j), (k, l)) in data if i == k]
# print(datahor)
dataver = [((i, j), (k, l)) for ((i, j), (k, l)) in data if j == l]
...
def makehorline(startend):
    ...
    return {(i, i2) for i2 in range(j, l + 1)} | {(i, i2) for i2 in range(l, j + 1)}


def makeverline(startend):
    (i, j), (k, l) = startend

    return {(i2, j) for i2 in range(i, k + 1)} | {(i2, j) for i2 in range(k, i + 1)}
