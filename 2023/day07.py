...
def handsortvalue(hand, use_wildcard_jokers):
    return (
        handtype(hand, use_wildcard_jokers),
        tuple(cardvalues[use_wildcard_jokers][c] for c in hand),
    )
...
for part in False, True:
    hands = [
        ...
        for i, (hand, bid) in enumerate([line.split() for line in data.splitlines()])
    ]
    print(hands)
    accum = 0
